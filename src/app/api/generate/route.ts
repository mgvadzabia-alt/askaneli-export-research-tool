...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    return NextResponse.json(
      { error: `additionalInstructions must be a string of at most ${MAX_INSTRUCTIONS_LENGTH} characters` },
      { status: 400 }
    );
  }
  ...
  const trimmedInstructions =
    typeof additionalInstructions === "string" && additionalInstructions.trim().length > 0
  ...
  generateResearchReport(trimmedCountry, trimmedProduct, reportLanguage, trimmedInstructions)
