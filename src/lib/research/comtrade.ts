...
function recordsToFindings(
  ...
  const findings: Omit<ResearchFinding, "id">[] = clean.map((r) => ({
    topic: "market size",
    claim: `Official UN Comtrade data: Georgia's ${commodityLabel} exports to ${partnerName} in ${r.year} were ${usd(r.value)} (${tonnes(r.kg)}).`,
    url: queryUrl,
  ...
  if (clean.length >= 2 && first.value > 0) {
    const pct = ((last.value - first.value) / first.value) * 100;
    const direction = pct >= 0 ? "up" : "down";
    findings.push({
      topic: "market size",
      claim: `Official UN Comtrade trend: Georgia's ${commodityLabel} exports to ${partnerName} went ${direction} ${Math.abs(pct).toFixed(0)}% from ${first.year} (${usd(first.value)}) to ${last.year} (${usd(last.value)}).`,
      url: queryUrl,
      sourceLabel: "UN Comtrade (official trade statistics)",
 ...
 * UN Comtrade and returns them as pre-verified "hard data" findings for the
 * research pass to build on. Returns an empty array (never throws) on any
 * failure, unknown country, or missing data.
 */
export async function fetchTradeFlowFindings(
  country: string
): Promise<Omit<ResearchFinding, "id">[]> {
  ...
  const commodities: Array<{ code: string; label: string }> = [
    { code: HS_WINE, label: "wine (HS 2204)" },
    { code: HS_SPIRITS, label: "spirits/brandy (HS 2208)" },
  ];
  ...
  for (const commodity of commodities) {
    ...
    findings.push(
      ...recordsToFindings(records, commodity.label, partner.name, citationUrl)
    );
  }
